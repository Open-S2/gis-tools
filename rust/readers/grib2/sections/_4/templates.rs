...
pub enum Grib2ProductDefinition {
    ...
    Grib2Template48(Grib2Template48),
}
impl Grib2ProductDefinition {
    ...
    pub fn new<T: Reader>(template: u16, reader: &T, sections: &Grib2Sections) -> Self {
        match template {
            ...
            8 => Grib2ProductDefinition::Grib2Template48(Grib2Template48::new(reader, sections)),
    ...
    pub fn values(&self) -> &TableCategory {
        match self {
            ...
            Grib2ProductDefinition::Grib2Template48(template) => &template.values,
        }
    }
}
...
impl Grib2Template48 {
    ...
    pub fn new<T: Reader>(reader: &T, sections: &Grib2Sections) -> Self {
        ...
        let unit_of_time_range_indicator = unit_of_time_range_indicator.into();
        // time
        let year = reader.uint16_be(Some(34));
        let month = reader.uint8(Some(36));
        let day = reader.uint8(Some(37));
        let hour = reader.uint8(Some(38));
        let minute = reader.uint8(Some(39));
        let second = reader.uint8(Some(40));
        // TODO: 41 onward
        ...
        Self {
            ...
            minute,
            second,
            time: Date::new_full(year, month, day, hour, minute, second),
        }
    }
}
...
pub fn calculate_forecast_time(ref_time: &Date, offset: i64, unit_of_time: &Grib2Table4_4) -> Date {
    match unit_of_time {
        ...
        Grib2Table4_4::Minute => Date::from_time(ref_time.get_time() + offset * 1000 * 60),
        Grib2Table4_4::Second => Date::from_time(ref_time.get_time() + offset * 1000),
        _ => {
            if cfg!(feature = "std") {
