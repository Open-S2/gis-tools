...
pub enum GTFSRealtimeScheduleRelationship {
...
pub struct GTFSRealtimeTripDescriptor {
    ...
    pub start_time: Option<String>, // 2 [string]
    ...
    /// In YYYYMMDD format.
    pub start_date: Option<Date>, // 3 [string]
    /// The relation between this trip and the static schedule. If a trip is done
    /// in accordance with temporary schedule, not reflected in GTFS, then it
    /// shouldn't be marked as SCHEDULED, but likely as ADDED.
    pub schedule_relationship: Option<GTFSRealtimeScheduleRelationship>, // 4 [enum]
    /// The route_id from the GTFS that this selector refers to.
    pub route_id: Option<String>, // 5 [string]
    ...
    pub modified_trip: Option<GTFSRealtimeModifiedTripSelector>, // 7 [message]
}
...
impl ProtoRead for GTFSRealtimeTripDescriptor {
    fn read(&mut self, tag: u64, pb: &mut Protobuf) {
        match tag {
            ...
            2 => self.start_time = Some(pb.read_string()),
            3 => self.start_date = Some(parse_gtfs_date(&pb.read_string()).unwrap_or_default()),
            4 => self.schedule_relationship = Some(pb.read_varint()),
            5 => self.route_id = Some(pb.read_string()),
            6 => self.direction_id = Some(pb.read_varint()),
            7 => {
                let mut modified_trip = GTFSRealtimeModifiedTripSelector::default();
                pb.read_message(&mut modified_trip);
                self.modified_trip = Some(modified_trip);
            }
...
pub struct GTFSRealtimeModifiedTripSelector {
    ...
    pub affected_trip_id: Option<String>, // 2 [string]
    /// The initially scheduled start time of this trip instance, applied to the frequency based
    /// modified trip. Same definition as start_time in TripDescriptor.
...
impl ProtoRead for GTFSRealtimeModifiedTripSelector {
    fn read(&mut self, tag: u64, pb: &mut Protobuf) {
        match tag {
            1 => self.modifications_id = Some(pb.read_string()),
            2 => self.affected_trip_id = Some(pb.read_string()),
            3 => self.start_time = Some(pb.read_string()),
            4 => self.start_date = Some(parse_gtfs_date(&pb.read_string()).unwrap_or_default()),
            _ => panic!("unknown tag {}", tag),
        }
    }
}
