...
use data_store::file::KEY_STORE_LENGTH;
use memmap2::Mmap;
use std::{
    cmp::Ordering,
    format,
...
impl SortedFile {
    ...
    pub fn new(input: &str) -> SortedFile {
        let file = OpenOptions::new().read(true).open(input).unwrap();
        let input = unsafe { Mmap::map(&file).unwrap() };
    ...
    fn read_buffer(&mut self, length: u64) -> Buffer {
        ...
        let input_bytes =
            self.input[self.offset as usize..(self.offset + length) as usize].to_vec();
