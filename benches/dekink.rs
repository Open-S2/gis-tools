...
use gistools::geometry::dekink_polygon;
use s2json::{NewXY, Point};

const COUNT: usize = 1_000_000;

fn dekink_polygon_test() {
    let polygon = vec![vec![
...
fn criterion_benchmark(c: &mut Criterion) {
    let mut group = c.benchmark_group("dekink_polygon_test");
    group.sample_size(10);
    group.bench_function("dekink", |b| b.iter(dekink_polygon_test));
    group.finish();
}
