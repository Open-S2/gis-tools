...
mod tests {
    ...
    fn from_vector_points() {
        let ortho = Orthodrome::from_points(
    ...
    fn intermediate_point_same() {
        let ortho = Orthodrome::new(0., 0., 0., 0.);
        assert_eq!(ortho.intermediate_point::<LonLat>(0.5), LonLat::new(0., 0., None));
    }
    ...
    fn intermediate_point_far() {
        ...
        assert_eq!(
            ortho.intermediate_point::<LonLat>(0.),
            LonLat::new(-59.99999999999999, -40., None)
        );
        assert_eq!(
        ...
        assert_eq!(
            ortho.intermediate_point::<LonLat>(0.4),
            LonLat::new(-21.692497560895635, -24.50037918247324, None)
        );
        assert_eq!(
            ortho.intermediate_point::<LonLat>(0.6),
            LonLat::new(-6.830669211476937, -13.564157442008685, None)
        );
        assert_eq!(
            ortho.intermediate_point::<LonLat>(0.8),
            LonLat::new(6.673353815433631, -1.8320330896428323, None)
        );
        assert_eq!(ortho.intermediate_point::<LonLat>(1.), LonLat::new(20., 10., None));
    }
