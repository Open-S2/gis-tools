import FileMultiMap from '../../../src/dataStore/kv/mmap';
...
// TODO: When FileMultiMap is working...
test.skip('parse basic case with mmap KV store', async () => {
  const fileReader = new FileReader(`${__dirname}/fixtures/test.pbf`);

  const reader = new OSMReader(fileReader, { removeEmptyNodes: false, store: FileMultiMap });
  const header = reader.getHeader();
  expect(header).toEqual({
    bbox: [-1, -1, -1, -1],
    optional_features: [],
