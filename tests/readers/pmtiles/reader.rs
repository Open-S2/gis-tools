...
mod tests {
    ...
    use alloc::vec;
    use gistools::{
        parsers::{BufferReader, FetchReader, FileReader},
    ...
    fn test_fixture_1() {
        smol::block_on(async {
            ...
            let tile = reader.get_tile(None, 0, 0, 0).await.unwrap();
            assert_eq!(
                tile,
                vec![
                    26, 47, 120, 2, 10, 21, 116, 101, 115, 116, 95, 102, 105, 120, 116, 117, 114,
                    101, 95, 49, 112, 109, 116, 105, 108, 101, 115, 40, 128, 32, 18, 17, 24, 3, 34,
                    13, 9, 150, 32, 232, 31, 26, 0, 24, 21, 0, 0, 23, 15,
                ]
            );
        });
    }
    ...
    #[test]
    fn test_server_s2() {
        // http://localhost:${server.port}/readers/pmtiles/fixtures/s2.s2pmtiles
        smol::block_on(async {
            let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            let path_str: String = path.to_str().unwrap().into();
            let server = spawn_test_server(&path_str);

            let url = format!("{}/tests/readers/pmtiles/fixtures/s2.s2pmtiles", server);
            ...
            assert_eq!(
                ...
                S2PMHeader {
                    ...
                    root_directory_length1: 5,
                    root_directory_offset2: 1,
                    root_directory_length2: 272,
                    root_directory_offset3: 273,
                    root_directory_length3: 5,
                    root_directory_offset4: 278,
                    root_directory_length4: 1,
                    root_directory_offset5: 279,
                    root_directory_length5: 1,
                    leaf_directory_offset1: 98339,
                    leaf_directory_length1: 0,
                    leaf_directory_offset2: 98339,
                    leaf_directory_length2: 0,
                    leaf_directory_offset3: 98339,
                    leaf_directory_length3: 0,
                    leaf_directory_offset4: 98339,
                    leaf_directory_length4: 0,
                    leaf_directory_offset5: 98339,
                    leaf_directory_length5: 0
                }
            );
