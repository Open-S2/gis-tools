...
describe('File Reader', () => {
  ...
  test('test_fixture_2', async () => {
    const fileReader = new FileReader(`${__dirname}/fixtures/test_fixture_2.pmtiles`);
    const testFixture2 = new S2PMTilesReader(fileReader);
...
test('mmap test_fixture_2', async () => {
  try {
    const mmapReader = new MMapReader(`${__dirname}/fixtures/test_fixture_2.pmtiles`);

    const testFixture2 = new S2PMTilesReader(mmapReader);
    const header = await testFixture2.getHeader();
    // header
    expect(header).toEqual({
      clustered: false,
