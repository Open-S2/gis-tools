...
mod tests {
    ...
    fn test_s2tiles_buffer_writer_wm() {
        ...
        assert_eq!(reader.get_tile_wm(0, 0, 0).unwrap(), buf);
        assert_eq!(reader.get_tile_wm(1, 0, 1).unwrap(), buf);
        assert_eq!(reader.get_tile_wm(9, 22, 9).unwrap(), buf2);
        assert!(!reader.has_tile_wm(1, 1, 1));
    }
    ...
    fn test_s2tiles_file_writer_s2() {
        ...
        assert_eq!(reader.get_tile_s2(0.into(), 0, 0, 0).unwrap(), buf);
        assert_eq!(reader.get_tile_s2(1.into(), 0, 0, 0).unwrap(), buf);
        assert_eq!(reader.get_tile_s2(3.into(), 2, 1, 1).unwrap(), buf2);
        assert_eq!(reader.get_tile_s2(4.into(), 5, 5, 5).unwrap(), buf2);
        assert_eq!(reader.get_tile_s2(5.into(), 5, 5, 5).unwrap(), buf);
        assert_eq!(reader.get_tile_s2(2.into(), 8, 1, 1).unwrap(), buf2);
        assert!(!reader.has_tile_s2(1.into(), 1, 1, 1));
    }
