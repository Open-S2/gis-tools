...
describe('cross', (): void => {
  const a: Point3D = [1, 2, 3];
  const b: Point3D = [1, 2, 3];
  const c: Point3D = [5, 6, 7];
  expect(cross(a, b)).toEqual([0, 0, 0]);
  expect(cross(a, c)).toEqual([-4, 8, -4]);
});
...
describe('subScalar', (): void => {
  it('should subtract 1 from each component of an XYZ point', (): void => {
    expect(subScalar([1, 2, 3], 1)).toEqual([0, 1, 2]);
  });
});

describe('sub', (): void => {
  it('should subtract an XYZ point from another XYZ point', (): void => {
    expect(sub([1, 2, 3], [1, 2, 3])).toEqual([0, 0, 0]);
  });
});
