...
mod tests {
    use gistools::proj::{
        CASSINI, CassiniProjection, CoordinateStep, Coords, Proj, ProjectCoordinates, derive_sphere,
    };
    ...
    fn test_cass() {
        ...
        // }
        let projection = CassiniProjection::new(proj);

        assert_eq!(projection.code(), CASSINI);
        assert_eq!(projection.name(), "Cassini");
        assert_eq!(
            CassiniProjection::names(),
    ...
    fn test_cass2() {
        ...
        let projection = CassiniProjection::new(proj.clone());
