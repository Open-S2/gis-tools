...
mod tests {
    use gistools::proj::{
        CoordinateStep, Coords, Proj, ProjectCoordinates, TransverseCentralCylindricalProjection,
        derive_sphere,
    };
    ...
    fn test_tcc() {
        ...
        // }
        let projection = TransverseCentralCylindricalProjection::new(proj);

        assert_eq!(projection.code(), -1);
        assert_eq!(projection.name(), "Transverse Central Cylindrical");
        assert_eq!(
            TransverseCentralCylindricalProjection::names(),
    ...
    fn test_tcc2() {
        ...
        let projection = TransverseCentralCylindricalProjection::new(proj.clone());
