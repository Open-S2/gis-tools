import { EARTH_RADIUS } from '../geometry';
...
export class PointIndexFast<T = Stringifiable> {
   ...
   */
  constructor(
    store: KDStoreConstructor<T> = KDSpatialIndex,
  ...
  *[Symbol.iterator](): Generator<KDKV<T>> {
  ...
  searchRadiusSphere(
    ...
    const pq = new PriorityQueue<NodeQuery<T>>([], (a, b) => a.dist - b.dist);
    // an object that represents the top kd-tree node (the whole Earth)
    let node: NodeQuery<T> | undefined = {
