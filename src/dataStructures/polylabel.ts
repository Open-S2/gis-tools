...
export default function polylabel(
  ...
  const cellQueue = new PriorityQueue<Cell>([], (a: Cell, b: Cell): number => b.max - a.max);
 ...
 * @returns - the cell
 */
function buildCell(x: number, y: number, h: number, polygon: VectorPolygon): Cell {
 ...
 */
function getCentroidCell(polygon: VectorPolygon): Cell {
  let area = 0;
  let x = 0;
  let y = 0;
