...
export interface DspaceOutput {
  ...
  inclm: number; // inclination
  xli: number;
  mm: number; // mean anomaly
  xni: number;
  nodem: number; // right ascension of ascending node
  dndt: number;
...
export function dspace(options: DspaceOptions, tsince: number): DspaceOutput {
  ...
  if (irez !== 0) {
    ...
    if (atime === 0.0 || tsince * atime <= 0.0 || Math.abs(tsince) < Math.abs(atime)) {
      atime = 0.0;
      xni = no;
      xli = xlamo;
    }
    ...
    while (iretn === 381) {
      ...
      if (Math.abs(tsince - atime) >= stepp) {
        iretn = 381;
      } else {
        ft = tsince - atime;
        iretn = 0;
