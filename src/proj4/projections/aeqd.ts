    ...
    if (this.sphere) {
      ...
      if (Math.abs(rh) <= EPSLN) {
        ...
        if (Math.abs(con) <= EPSLN) {
          if (this.lat0 >= 0) {
            lon = adjustLon(this.long0 + Math.atan2(p.x, -p.y));
          } else {
            lon = adjustLon(this.long0 - Math.atan2(-p.x, p.y));
          }
        } else {
        ...
        if ((Math.abs(con) < EPSLN) && (Math.abs(p.x) < EPSLN)) {
          ...
          lon = adjustLon(this.long0 + Math.atan2((p.x * sinz * this.cos_p12), (con * rh)));
        }*/
          lon = adjustLon(
            this.long0 +
      ...
      if (Math.abs(this.sin_p12 - 1) <= EPSLN) {
        ...
        lon = adjustLon(this.long0 + Math.atan2(p.x, -1 * p.y));
        p.x = lon;
        p.y = lat;
        return p;
      } else if (Math.abs(this.sin_p12 + 1) <= EPSLN) {
        ...
        lon = adjustLon(this.long0 + Math.asin((Math.sin(Az) * Math.sin(Ee)) / Math.cos(psi)));
        sinpsi = Math.sin(psi);
        lat = Math.atan2(
