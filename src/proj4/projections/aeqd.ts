...
export class AzimuthalEquidistant extends ProjectionBase implements ProjectionTransform {
   ...
   * @returns - an Azimuthal Equidistant point
   */
  forward(p: VectorPoint): VectorPoint {
    ...
    if (this.sphere) {
      if (abs(this.sin_p12 - 1) <= EPSLN) {
        ...
        //default case
        cos_c = this.sin_p12 * sinphi + this.cos_p12 * cosphi * cos(dlon);
        c = acos(cos_c);
        kp = c ? c / sin(c) : 1;
        p.x = this.x0 + this.a * kp * cosphi * sin(dlon);
        p.y = this.y0 + this.a * kp * (this.cos_p12 * sinphi - this.sin_p12 * cosphi * cos(dlon));
        return p;
      }
    } else {
      e0 = e0fn(this.es);
      ...
      if (abs(this.sin_p12 - 1) <= EPSLN) {
        ...
        psi = atan((1 - this.es) * tanphi + (this.es * Nl1 * this.sin_p12) / (Nl * cosphi));
        Az = atan2(sin(dlon), this.cos_p12 * tan(psi) - this.sin_p12 * cos(dlon));
        if (Az === 0) {
          s = asin(this.cos_p12 * sin(psi) - this.sin_p12 * cos(psi));
        } else if (abs(abs(Az) - PI) <= EPSLN) {
          s = -asin(this.cos_p12 * sin(psi) - this.sin_p12 * cos(psi));
        } else {
          s = asin((sin(dlon) * cos(psi)) / sin(Az));
        }
        G = (this.e * this.sin_p12) / sqrt(1 - this.es);
        H = (this.e * this.cos_p12 * cos(Az)) / sqrt(1 - this.es);
        GH = G * H;
        Hs = H * H;
        s2 = s * s;
        s3 = s2 * s;
    ...
    if (this.sphere) {
      ...
      if (abs(rh) <= EPSLN) {
        ...
        if ((abs(con) < EPSLN) && (abs(p.x) < EPSLN)) {
          ...
          var temp = atan2((p.x * sinz * this.cos_p12), (con * rh));
          lon = adjustLon(this.long0 + atan2((p.x * sinz * this.cos_p12), (con * rh)));
        }*/
          lon = adjustLon(
            this.long0 + atan2(p.x * sinz, rh * this.cos_p12 * cosz - p.y * this.sin_p12 * sinz),
          );
        }
      }
      ...
      if (abs(this.sin_p12 - 1) <= EPSLN) {
        ...
        psi = asin(this.sin_p12 * cos(Ee) + this.cos_p12 * sin(Ee) * cosAz);
        lon = adjustLon(this.long0 + asin((sin(Az) * sin(Ee)) / cos(psi)));
        sinpsi = sin(psi);
        lat = atan2((sinpsi - this.es * F * this.sin_p12) * tan(psi), sinpsi * (1 - this.es));
        p.x = lon;
        p.y = lat;
        return p;
      }
