...
import {EPSLN, TWO_PI, SPI, HALF_PI, QUART_PI} from '../constants/values';
...
export function init() {
  ...
  } else if (Math.abs(this.long0) <= QUART_PI) {
    this.face = FACE_ENUM.FRONT;
  } else if (Math.abs(this.long0) <= HALF_PI + QUART_PI) {
...
export function forward(p) {
  ...
  mu = Math.atan((12 / SPI) * (theta + Math.acos(Math.sin(theta) * Math.cos(QUART_PI)) - HALF_PI));
