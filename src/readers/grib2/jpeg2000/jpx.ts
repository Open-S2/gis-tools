...
export class JpxImage {
   ...
   */
  parse(data: Reader) {
    const head = data.getUint16(0);
    // No box header, immediate start of codestream (SOC)
    if (head === 0xff4f) {
      this.parseCodestream(data, 0, data.byteLength);
      return;
    }
    ...
    while (position < length) {
      let headerSize = 8;
      let lbox = data.getUint16(position);
      const tbox = data.getUint16(position + 4);
      position += headerSize;
      if (lbox === 1) {
        // XLBox: read UInt64 according to spec.
        // JavaScript's int precision of 53 bit should be sufficient here.
        lbox = data.getUint16(position) * 4294967296 + data.getUint16(position + 4);
        position += 8;
        headerSize += 8;
      }
      if (lbox === 0) {
      ...
      switch (tbox) {
        ...
        case 0x636f6c72: {
          ...
          const method = data.getUint8(position);
          if (method === 1) {
            // enumerated colorspace
            const colorspace = data.getUint32(position + 3);
            switch (colorspace) {
        ...
        case 0x6a502020: // 'jP\024\024'
          if (data.getUint32(position) !== 0x0d0a870a) {
   ...
   * @param end
   */
  parseCodestream(data: Reader, start: number, end: number) {
    const context = {};
      ...
      while (position + 1 < end) {
        ...
        switch (code) {
          ...
          case 0xff51: {
            ...
            const siz = {};
            siz.Xsiz = data.getUint32(position + 4);
            siz.Ysiz = data.getUint32(position + 8);
            siz.XOsiz = data.getUint32(position + 12);
            siz.YOsiz = data.getUint32(position + 16);
            siz.XTsiz = data.getUint32(position + 20);
            siz.YTsiz = data.getUint32(position + 24);
            siz.XTOsiz = data.getUint32(position + 28);
            siz.YTOsiz = data.getUint32(position + 32);
            const componentsCount = data.getUint16(position + 36);
          ...
          case 0xff52: // Coding style default (COD)
            ...
            cod.layersCount = data.getUint16(j);
            j += 2;
          ...
          case 0xff90: // Start of tile-part (SOT)
            length = data.getUint16(position);
            tile = {};
            tile.index = data.getUint16(position + 2);
            tile.length = data.getUint32(position + 4);
...
function buildPrecincts(context, resolution, dimensions) {
  ...
  const numprecinctswide =
    resolution.trx1 > resolution.trx0
      ? Math.ceil(resolution.trx1 / precinctWidth) - Math.floor(resolution.trx0 / precinctWidth)
      : 0;
  const numprecinctshigh =
    resolution.try1 > resolution.try0
      ? Math.ceil(resolution.try1 / precinctHeight) - Math.floor(resolution.try0 / precinctHeight)
      : 0;
 ...
 * @param pxIndex
 * @param pyIndex
 * @param sizeInImageScale
 * @param precinctIterationSizes
 * @param resolution
 */
...
class Transform {
   ...
   * @param size
   */
  extendextend(buffer, offset, size) {
   ...
   * @param v0
   */
  iterateTransform_iterate(ll, hl_lh_hh, u0, v0) {
 ...
 * @param length
 */
function irreversibleTransformFilter(x: number[], offset: number, length: number) {
  const len = length >> 1;
  offset = offset | 0;
 ...
 * @param length
 */
function reversibleTransformFilter(x: number[], offset: number, length: number) {
  const len = length >> 1;
  offset = offset | 0;
  let j, n;
