...
const JpegImage = (function jpegImage() {
  ...
  constructor.prototype = {
     ...
     * @param data
     */
    parse: function parse(data: Uint8Array): void {
      const maxResolutionInPixels = this.opts.maxResolutionInMP * 1000 * 1000;
      let offset = 0;
      /** @returns - a uint16 value  at offset */
      function readUint16(): number {
        const value = (data[offset] << 8) | data[offset + 1];
        offset += 2;
        return value;
      }
      /** @returns - a data block at offset */
  ...
  function requestMemoryAllocation(increaseAmount = 0) {
    ...
    if (totalMemoryImpactBytes > maxMemoryUsageBytes) {
      const exceededAmount = Math.ceil(
        (totalMemoryImpactBytes - maxMemoryUsageBytes) / 1024 / 1024,
      );
...
export function jpegDecoder(buffer: ArrayBufferLike, jpegTables?: number[]): ArrayBufferLike {
  if (jpegTables !== undefined) {
    const buf = Buffer.from(buffer);
    const jpegTablesBuffer = Buffer.from(jpegTables);
    const newBuffer = Buffer.concat([jpegTablesBuffer, buf]);
    buffer = newBuffer.buffer;
  }
  const res = decode(buffer, { jpegTables });
