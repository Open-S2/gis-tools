...
import { makeCustomSource, makeRemoteSource } from './source/remote';

import { ARRAY_FIELDS, FIELD_TAG_NAMES, FIELD_TYPES, geoKeyNames } from './globals';

/**
 * @param fieldType
 */
function getFieldTypeLength(fieldType): 1 | 2 | 4 | 8 {
  switch (fieldType) {
...
function parseGeoKeyDirectory(fileDirectory) {
  ...
  for (let i = 4; i <= rawGeoKeyDirectory[3] * 4; i += 4) {
    ...
    const location = rawGeoKeyDirectory[i + 1] ? FIELD_TAG_NAMES[rawGeoKeyDirectory[i + 1]] : null;
...
function getValues(dataSlice, fieldType, count, offset) {
  ...
  // normal fields
  if (!(fieldType === FIELD_TYPES.RATIONAL || fieldType === FIELD_TYPES.SRATIONAL)) {
 ...
 * in the file.
 */
export class GeoTIFFImageIndexError extends Error {
  /**
   * @param index
   */
  constructor(index) {
...
class GeoTIFFBase {
   ...
   * @returns the decoded array(s), with `height` and `width`, as a promise
   */
...
/**
 * [cache=false] whether or not decoded tiles shall be cached.
 */
...
class GeoTIFF extends GeoTIFFBase {
  ...
  async parseFileDirectoryAt(offset) {
    ...
    for (let entryCount = 0; entryCount < numDirEntries; i += entrySize, ++entryCount) {
      ...
      fileDirectory[FIELD_TAG_NAMES[fieldTag]] = value;
    }
   ...
   */
  async getImage(index = 0): Promise<GeoTIFFImage> {
   ...
   * @returns the parsed ghost area or null, if no such area was found
   */
  async getGhostValues() {
    ...
    if (detectionString === getValues(slice, FIELD_TYPES.ASCII, detectionString.length, offset)) {
      const valuesString = getValues(slice, FIELD_TYPES.ASCII, heuristicAreaSize, offset);
      const firstLine = valuesString.split('\n')[0];
      const metadataSize = Number(firstLine.split('=')[1].split(' ')[0]) + firstLine.length;
      if (metadataSize > heuristicAreaSize) {
        slice = await this.getSlice(offset, metadataSize);
      }
      const fullString = getValues(slice, FIELD_TYPES.ASCII, metadataSize, offset);
      this.ghostValues = {};
      fullString
        .split('\n')
...
export class MultiGeoTIFF extends GeoTIFFBase {
   ...
   * @param mainFile The main GeoTIFF file.
   * @param overviewFiles An array of overview files.
   */
  ...
  async parseFileDirectoriesPerFile() {
    const requests = [this.mainFile.parseFileDirectoryAt(this.mainFile.firstIFDOffset)].concat(
      this.overviewFiles.map((file) => file.parseFileDirectoryAt(file.firstIFDOffset)),
    );
 ...
 * @param url The URL to access the image from
 * @param [options] Additional options to pass to the source.
 *                           See {@link makeRemoteSource} for details.
 * @param [signal] An AbortSignal that may be signalled if the request is
 *                               to be aborted
 * @returns The resulting GeoTIFF file.
 */
 ...
 * @param arrayBuffer The data to read the file from.
 * @param [signal] An AbortSignal that may be signalled if the request is
 *                               to be aborted
 * @returns The resulting GeoTIFF file.
 */
 ...
 * object.
 * @param blob The Blob or File object to read from.
 * @param [signal] An AbortSignal that may be signalled if the request is
 *                               to be aborted
 * @returns The resulting GeoTIFF file.
 */
 ...
 * @param mainUrl The URL for the main file.
 * @param overviewUrls An array of URLs for the overview images.
 * @param [options] Additional options to pass to the source.
 *                           See [makeRemoteSource]{@link module:source.makeRemoteSource}
 *                           for details.
 * @param [signal] An AbortSignal that may be signalled if the request is
 *                               to be aborted
 * @returns The resulting MultiGeoTIFF file.
 */
